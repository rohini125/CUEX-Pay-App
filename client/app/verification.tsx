...
type VerificationScreenProps = {
  navigation: StackNavigationProp<RootStackParamList, 'FrontPage'>;
};

const VerificationPage: React.FC<VerificationScreenProps> = ({ navigation }) => {
  const [otp, setOtp] = useState('');
  const [timer, setTimer] = useState(30);
  const router = useRouter(); // Correct way to use navigation in Expo Routermi 
  useEffect(() => {
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}>
      ...
      <Text style={{ fontSize: 16, color: 'red', marginBottom: 10 }}>Time left: {timer}s</Text>
      <TextInput
        style={{
          borderWidth: 1,
          borderColor: '#ccc',
